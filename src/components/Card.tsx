...
export default function Card({ project }: { project: Project }) {
  ...
  return (
    <div
      style={{ backgroundImage: `src/assets/projects/${project.image}` }}
      className={`${
        sizes[project.size]
      } relative bg-purple-100 flex items-end rounded-[36px] overflow-hidden 
    ...
    >
      ...
      <div className="absolute top-8 left-11">
        <p className="text-card-title">{project.title}</p>
        <p className="text-card-text">{project.text}</p>
      </div>
      <div
      ...
      >
        <p className="text-card-desc">{project.description}</p>
